...
R does not use Excel file format; everything needs to be saved as a csv. It is also good practice not to use spaces in filenames because you will soon be entering a command line environment.
...
### Chapter 1 - Section A 
Caterpillar Engine Data cat_engine.r based on the caterpillar engine data. takes arguments engine data files and start times. outputs raw engine data and an interpolated time series (zoo objects). note: this code handles both the wireless data and the remote data and attempts to combine the two at the end (based on timestamps)

Example R code 1-A Set the working directory to where the engine data csv file lives.
```{r}
...
### Chapter 1 - Section B 
Liebherr Engine Data L_engine.r Based on the Liebherr engine data. takes arguments engine data file, start time (for each run) and date. outputs raw engine data and an interpolated time series (zoo object). note: the user must enter the start and stop times of each run that is in the engine data. see below for an example.

Example R code 1-B Set the working directory to where the engine data csv file lives. 
```r
...
start <- 96060+21*60+37
```

Taking our start/stop times for each run 
```r
run1_start <- 0 run1_stop <- (116060+0*60)-start 
```
...
### Chapter 1 - Section C
Komatsu engine data K_engine.r Based on the Komatsu engine data. Please save each tab as its own run. takes arguments engine data file, start time (for each run) and date. outputs raw engine data and an interpolated time series (zoo object). note: the user must enter the start and stop times of each run that is in the engine data. see below for an example.

Example R code 1-C Set the working directory to where the engine data csv file lives.
```r
...
start <- 96060+39*60+29
```

By inspection, find the length of the good data. I can't think of a way around this. 
```r
df <- df[1:6719,]
```
...
```r
start1 <- 3368 end1 <- 8724 
```

Start/stop rows for run2 
```r
start2 <- 8753 end2 <- 15090-5
```
...
If you're close (or not) start with different values for start1/end1 
```r
...
```r
start_gps <- as.POSIXct(96060+26*60+47, origin="2013-06-02")
```
...
Cutting up engine zoo object and plotting
```r
...
summary(hnu_part) plot.zoo(hnu_part) 
dev.new() 
plot.zoo(hnu)
```
